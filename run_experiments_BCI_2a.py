...
def getModel(model_name, use_cpu):
    ...
    if(model_name == 'EEGNet_fusion_V2'):
        model = EEGModels.EEGNet_enhanced_fusion4(nb_classes = 4, Chans = 22, Samples=1125, cpu=use_cpu) 
        ...
        model = EEGModels.ShallowConvNet(nb_classes=4, Chans = 22, Samples = 1125, cpu=use_cpu)
    else:
...
def run():
    # Get dataset path
    data_path = 'EEGMotorImagery-master/BCI Competition IV/'
