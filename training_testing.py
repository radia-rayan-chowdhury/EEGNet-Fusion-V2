...
def run_experiment(X, y, experiment, use_cpu=False, test_model=True):
    ...
    if test_split == 0:
        X_train, X_val, y_train, y_val = train_test_split(X, y, test_size=experiment.get_val_split(), random_state=42, shuffle = False)
        X_test, y_test = [], []
        test_model = False
    else:
        X_train_val, X_test, y_train_val, y_test = train_test_split(X, y, test_size=test_split, random_state=42, shuffle = False)
        X_train, X_val, y_train, y_val = train_test_split(X_train_val, y_train_val,
                                                          test_size=experiment.get_val_split(), random_state=42, shuffle = False)
